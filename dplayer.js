...
function mzPlayer(){
	...
	if(arguments[0].video.srctype=='simple'||!arguments[0].video.srctype){
		solo=1;
		html='<video class="player" src="'+arguments[0].video.src+'" webkit-playsinline playsinline preload="auto" poster="'+pic+'" type="video/mp4"></video>';
	}
	if(arguments[0].video.srctype=='json'){
		var url=arguments[0].video.src;
		xhr=$.ajax({url:arguments[0].video.src,async:false});
  		list=jQuery.parseJSON(xhr.responseText)['video'];
		vTime=new Array();
		var alltime = 0;
		for(i=0;i<list.length;i++)
		{	alltime+=parseFloat(list[i]['seconds']);
			vTime[i]=alltime;
		}		
	}
$(container).append("<div class='mzplayer'></div>");
$(".mzplayer").append("<div class='loading'><img src='logo.png' width='420' height='220' /></div>");
...
$(".r-menu").append('<div class="r-menu-item"><a target="_new" href="about:blank#blocked" style="text-decoration:none;" target="_blank">百度一下</a></div>');
...
function allStop(){
	if(solo==1)
		return;
	if(list.length>1){
		$(".video-wrap").html('<video class="player" src="'+list[0]['file']+'" webkit-playsinline playsinline preload="auto" poster="'+pic+'" type="video/mp4"></video><video class="player" src="'+list[1]['file']+'" webkit-playsinline playsinline preload="auto" style="display: block;z-index:-5;" type="video/mp4"></video>');
}
}
//播放时刷新函数 分段视频专用
function whenPlay(){
	try {
		...
		if($(".player")[0].src==list[list.length-1]['file']){           //最后一个
			...
			if(getWhich()!=list.length-1)
				$(".video-wrap").append('<video class="player" src="'+list[getWhich()+1]['file']+'" webkit-playsinline playsinline preload="auto" style="display: block;z-index:-5;" type="video/mp4"></video>');
		}
	}
